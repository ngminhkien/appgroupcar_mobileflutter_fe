...
import '../cubit/shared_ride_detail_cubit.dart';
import '../cubit/shared_ride_detail_state.dart';
import '../models/trip_detail_navigation_args.dart';

class SharedRideDetailScreen extends StatelessWidget {
  const SharedRideDetailScreen({super.key, required this.args});
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (_) => sl<SharedRideDetailCubit>()..initialize(args),
        ),
        BlocProvider(create: (_) => sl<BusBookingActionCubit>()),
      ],
      child: const _SharedRideDetailView(),
    );
  }
}

class _SharedRideDetailView extends StatelessWidget {
  const _SharedRideDetailView();
    ...
    return BlocListener<BusBookingActionCubit, BusBookingActionState>(
      ...
      listener: (context, bookingState) {
        ...
        if (bookingState.status == BusBookingActionStatus.success &&
          ...
          const message = 'Dat chuyen xe ghep thanh cong';
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text(
            'Chi tiet chuyen xe ghep',
            style: TextStyle(fontWeight: FontWeight.w700),
          ),
        ),
        body: BlocBuilder<SharedRideDetailCubit, SharedRideDetailState>(
          builder: (context, state) {
            switch (state.status) {
              case SharedRideDetailStatus.loading:
                return const _DetailLoadingView();
              case SharedRideDetailStatus.failure:
                return _DetailErrorView(
                  message: state.errorMessage,
                  onRetry: () =>
                      context.read<SharedRideDetailCubit>().retryDetail(),
                );
              case SharedRideDetailStatus.unsupported:
                return _UnsupportedServiceView(
                  serviceCode: state.serviceCode,
                  message: state.errorMessage,
                );
              case SharedRideDetailStatus.success:
              ...
              case SharedRideDetailStatus.initial:
...
class _DetailSuccessView extends StatelessWidget {
  ...
  final SharedRideDetailState state;
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              if (detail.availableSeats != null)
                _InfoChip(
                  icon: Icons.event_seat_outlined,
                  label:
                      'Con trong: ${detail.availableSeats}/${detail.totalSeats}',
                  highlighted: true,
                ),
            ],
          ),
...
class _RouteSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (route == null) {
      return const _MissingSection(message: 'Chua co thong tin hanh trinh');
    }
    final sortedPoints = route!.routePoints.toList()
      ..sort((a, b) => a.sequence.compareTo(b.sequence));

    SharedRideRoutePointDetail? pickupPoint;
    SharedRideRoutePointDetail? dropoffPoint;
...
class _RoutePointsDropdown extends StatelessWidget {
  ...
  final List<SharedRideRoutePointDetail> points;
...
class _RoutePointItem extends StatelessWidget {
  ...
  final SharedRideRoutePointDetail point;
...
class _BookingActionCard extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<SharedRideRoutePointDetail>(
            ...
            items: pickupPoints.map((point) {
              return DropdownMenuItem<SharedRideRoutePointDetail>(
                value: point,
                child: Text(
            ...
            onChanged: (val) =>
                context.read<SharedRideDetailCubit>().selectPickupLocation(val),
          ),
          SizedBox(height: 10.h),
          DropdownButtonFormField<SharedRideRoutePointDetail>(
          ...
          Text(
            state.selectedPickupLocation == null ||
                ...
                : 'Chuyen di: ${state.selectedPickupLocation!.locationName} -> ${state.selectedDropoffLocation!.locationName}',
            style: TextStyle(
              color: AppColors.onSurfaceVariant,
              fontSize: 12.sp,
            ),
          ...
          if (state.selectedPickupLocation != null &&
            ...
            SizedBox(height: 8.h),
            Text(
              'Gia cuoc: ${_formatMoney(detail.price)}',
              style: TextStyle(
              ...
              builder: (context, bookingState) {
                ...
                return SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: canBook ? () => _bookOffer(context) : null,
                    child: isSubmitting
                        ? SizedBox(
                            width: 18.w,
                            height: 18.w,
  ...
  void _bookOffer(BuildContext context) {
    if (state.selectedPickupLocation == null ||
        state.selectedDropoffLocation == null) {
      return;
    }
    context.read<BusBookingActionCubit>().createOfferBooking(
      offerId: detail.id,
...
class _DetailErrorView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final text =
        ...
        'Khong the tai chi tiet chuyen xe ghep';
    return Center(
      child: Padding(
